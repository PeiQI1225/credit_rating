...
def BinBadRate(df, col, target, grantRateIndicator=0):
    ...
    #regroup将记录特征col中每个取值对应的total值和bad值
...
def BadRateEncoding(df, col, target):
    ...
    badRateEnconding = df[col].apply(lambda x: br_dict[x] if not np.isnan(x) else x)
...
def ChiMerge(df, col, target, max_interval=5,special_attribute=[],minBinPcnt=0):
    ...
    if N_distinct <= max_interval:  #如果原始属性的取值个数低于max_interval，不执行这段函数
        ...
        # 查看特殊值，如果有，去掉特殊值分箱；没有继续分箱
        df1 = df.loc[df[col].isin(special_attribute) | np.isnan(df[col])]
        df2 = df.loc[~(df[col].isin(special_attribute) | np.isnan(df[col]))]
...
def Chi2(df, total_col, bad_col):
    ...
    goodChi = [(i[0]-i[1])**2/i[0] if i[0]!=0 else 0 for i in goodCombined]
...
def SplitData(df, col, numOfSplit, special_attribute=[]):
    ...
    N = df2.shape[0]
    n = N/numOfSplit
    splitPointIndex = [math.ceil(i*n) for i in range(1,numOfSplit)]
    rawValues = sorted(list(df2[col]))
    splitPoint = [rawValues[i] for i in splitPointIndex]
    splitPoint = sorted(list(set(splitPoint)))
    return splitPoint

def AssignBin(x, cutOffPoints,special_attribute=[],return_nan_flag = True):
    '''
    :param x: 某个变量的某个取值
    ...
    if np.isnan(x) and return_nan_flag: 
        return np.nan 
    if x<=cutOffPoints[0]:
        return 'Bin 0'
...
for col in numerical_vars:
    print("bining for variable", col)
    new_var = str(col) + '_numBin'
    colLevels = sorted(list(set(df[col])))
    #print(colLevels)
    N_distinct = len(colLevels)
    if N_distinct <= max_interval:  #如果原始属性的取值个数低于max_interval，不执行这段函数
            ...
            while minPcnt < minBinPcnt and len(cutOffPoints) > 2:
                ...
                # 如果占比最小的箱是中间的某一箱，则需要和前后中的一个箱进行合并，依据是较小的卡方值
                    ...
                    if chisq1 < chisq2:
                        ...
                        cutOffPoints.remove(cutOffPoints[currentIndex])
        cutOffPoints = special_attribute + cutOffPoints        
        print('cut-off points for variabel',col,'are',cutOffPoints)
        mapped_new_var = df[col].map(lambda x: AssignBin(x, cutOffPoints,special_attribute=special_attribute))
        
    print(cutOffPoints)
    print(list(df[col].loc[df[col].isin(special_attribute) | np.isnan(df[col])]))
    print(list(mapped_new_var.loc[df[col].isin(special_attribute) | np.isnan(df[col])]))
